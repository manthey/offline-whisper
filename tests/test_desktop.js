...
const testAudioPath2 = path.join(__dirname, 'test_audio_2.wav');
const EXPECTED_PHRASE = 'this is a test';
const EXPECTED_PHRASE_2 = 'offline whisper';
let lastStatus = 0;
...
async function run() {
  ...
  for (let audioIdx = 0; audioIdx < 2; audioIdx += 1) {
    ...
    const audioData = readWavAsFloat32(audioIdx ? testAudioPath2 : testAudioPath);
    ...
    const expected = audioIdx ? EXPECTED_PHRASE_2 : EXPECTED_PHRASE;
    console.log('Expected:    "' + expected + '"');
    if (!transcribedText.includes(expected)) {
